...
extern "C" void extractAssets(JNIEnv* env, jclass cls) {
    ...
    jmethodID getStorageDirectory = env->GetStaticMethodID(cls,"getStorageDirectory","()Ljava/lang/String;");
    jstring jstorage = (jstring)env->CallStaticObjectMethod(cls, getStorageDirectory);
    const char* storage_dir = env->GetStringUTFChars(jstorage, NULL);
    _AndroidStorageDir = storage_dir;
    env->ReleaseStringUTFChars(jstorage, storage_dir);
